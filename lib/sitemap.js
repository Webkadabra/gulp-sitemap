...
/**
 * config[xlsStylesheet]
 */
function _xml_header(config) {
	var header = [
    '<?xml version="1.0" encoding="UTF-8"?>',
	];
	if (config.xlsStylesheet) {
		header.push(config.xlsStylesheet);
	}
	header.push('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">');
	return header;
}
 ...
 * config[xlsStylesheet]
 */
function _xml_headerHref(config) {
	var headerHref = [
		'<?xml version="1.0" encoding="UTF-8"?>',
	];
	if (config.xlsStylesheet) {
		headerHref.push(config.xlsStylesheet);
	}
	headerHref.push('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml">');
	return headerHref;
}
...
function prepareSitemap(entries, config) {
    ...
    return (entriesHref ? (_xml_headerHref(config)) : (_xml_header(config)))
